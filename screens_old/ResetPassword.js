...
const Login = ({
  ...
  const resetPassword = () => {
    if (!code.trim()) {
      setErrorCode(`The Code can't be empty`);
    } else {
      setErrorCode();
    }
    if (!password) {
      setErrorPassword(`The Password can't be empty`);
    } else {
      setErrorPassword();
    }
    console.log(email);
  ...
  return (
    ...
    >
      ...
      {status === 1 && <>
        <Text multiline style={styles.info}>A verification code has been sent to your email address: {destination}</Text>
        <Input
          inputContainerStyle={styles.input}
