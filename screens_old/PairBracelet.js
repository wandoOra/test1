...
const PairBracelet = ({
  ...
  const onFound = (wandoTagId) => {
    ...
    if (existed) {
      setStatus('error');
      setMessage('Wristband is already registered to this user previously.'.toLowerCase())
    } else {
      ...
      >
        ...
        <View style={{alignItems: 'center'}}>
          ...
          {status === 'normal' && <>
            ...
            <Text style={{textAlign: 'center', marginTop: 64, color: 'white'}}>hold the bracelet close to your phone.</Text>
          </>}
          {status === 'error' && <>
