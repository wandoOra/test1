...
const Signup = ({
  ...
  const signup = async () => {
    if (!firstName.trim()) {
      ...
      return setErrorFN(`The First name can't be empty`);
    } else {
      setErrorFN();
    }
    if (!lastName.trim()) {
      ...
      return setErrorLN(`The Last name can't be empty`);
    } else {
      setErrorLN();
    }
    if (!email.trim()) {
    ...
    if (!password) {
      ...
      scrollViewRef.current.scrollTo({y: height * 0.8, animated: true})
      passwordRef.current.focus();
      return setErrorPassword(`The Password must be at least 8 characters long`);
    } else {
      setErrorPassword();
    }
  ...
  return (
    ...
    >
      ...
      <Text h3 style={{...styles.title}}>
        ...
        {status === 'created' && 'congratulations! your account was successfully created.'}
      </Text>
      {status === 'signup' && <>
        <Input
          ref={firstNameRef}
        ...
        />
        <Text style={{alignSelf: 'flex-start', marginLeft: 12}}>phone</Text>
        <PhoneInput
          ref={phoneRef}
          // defaultValue={value}
      ...
      <View style={styles.commandbar}>
        ...
        <TouchableOpacity onPress={() => {navigation.navigate('Login')}}>
          <Text style={{fontWeight: 'bold'}}>already have an account? just login</Text>
        </TouchableOpacity>
      </View>
      <EmptyGap />
    </BasicScreen>
